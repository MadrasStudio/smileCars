...
// var filterBtnStyle = getComputedStyle(filterBtn);
var carlistContent = document.getElementById("content");
var sticky = filterBtn ? filterBtn.offsetTop : sticky;

jQuery(window).scroll(function () {
    if ($().sticky) {
        $('.header.fixed').sticky('update');
    }
    if (filterBtn && window.pageYOffset > carlistContent.offsetHeight - 100) {
        filterBtn.classList.remove('d-block');
        ...
        if (window.pageYOffset >= sticky) 
            ...
            filterBtn.classList.remove('sticky');
        filterBtn.classList.add('d-block');
...
// submitted values will be pre-poulated on Checkout page
function submitForm() {
    const homeFormValues = { 
        pickup: document.forms["homeForm"]["pickup"].value,
        pickupDate: document.forms["homeForm"]["pickupDate"].value,
        pickupTime: document.forms["homeForm"]["pickupTime"].value,
        dropOff: document.forms["homeForm"]["dropOff"].value,
    }
    sessionStorage.setItem("userValue", JSON.stringify(homeFormValues));
    window.location.href = "car-listing.html";
}



// making filter link button as sticky header
