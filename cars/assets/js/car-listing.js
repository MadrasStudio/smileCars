        ...
        for (let j = 0; j < temp[i].cars.length; j++) {
            content += `<div class="thumbnail no-border no-padding thumbnail-car-card clearfix">
                            ...
                            <div class="caption">
                                ...
                                <h5 class="caption-title-sub">Start from ${temp[i].cars[j].detailedInfo.dayRate}&#x20b9;/Day</h5>
                                <div class="caption-text m-h-0 m-b-5"></div>
                                <table class="table">
                                    <tr>
                                        <td class="w-35"><i class="fa fa-dashboard"></i><br> ${temp[i].cars[j].variant}</td>
                                        <td class="w-35"><i class="fa fa-cog"></i><br> ${temp[i].cars[j].mode}</td>
...
function clearFilter() {
    ...
    $('.bootstrap-select .filter-option').text('All');
    //Check the selected attribute for the real select
    $('select[name=selValue]').val('All');
    getCarsList("");
}
